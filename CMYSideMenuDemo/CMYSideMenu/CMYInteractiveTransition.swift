//
//  CMYInteractiveTransition.swift
//  CMYSideMenu
//
...
enum CMYSideMenuShowType:Int {
    case show,hidden
}

class CMYInteractiveTransition: NSObject,UIViewControllerAnimatedTransitioning {
    
    var type:CMYSideMenuShowType!
    var config:CMYSideMenuConfig!
    
    init(showType: CMYSideMenuShowType,config: CMYSideMenuConfig) {
        super.init()
        type = showType
        self.config = config
    }
    ...
    func animateTransitionShowType(transitionContext: UIViewControllerContextTransitioning) {
        ...
        var mask:CMYSideMenuMaskView?
        for view in toController.view.subviews {
            if view.isKind(of: CMYSideMenuMaskView.classForCoder()){
                mask = view as? CMYSideMenuMaskView
                break
            }
        }
        if mask == nil {
            mask = CMYSideMenuMaskView()
    ...
    func animateTransitionHiddenType(transitionContext: UIViewControllerContextTransitioning) {
        ...
        let flag: CGFloat! = config.direction == CMYSideMenuDirection.left ? -1.0 : 1.0
        var fromTransform:CGAffineTransform = CGAffineTransform.init(translationX: flag * width, y: 0)
